...
nuscenes_subcategories = {
    'noise': [
        'Any lidar return that does not correspond to a physical object, such as dust, vapor, noise, fog, raindrops, smoke and reflections.'],  # 0, -1
    'animal': ["animal", 'cat', 'dog', 'rat', 'deer', 'bird'],  # 1, -1
    'human.pedestrian.adult': ['Adult.'],  # 2, 7
    'human.pedestrian.child': ['Child.'],  # 3, 7
    'human.pedestrian.construction_worker': ['Construction worker'],  # 4, 7
    'human.pedestrian.personal_mobility': ['skateboard', 'segway'],  # 5, -1
    'human.pedestrian.police_officer': ['Police officer.'],  # 6, 7
    'human.pedestrian.stroller': ['Stroller'],  # 7, -1
    'human.pedestrian.wheelchair': ['Wheelchair'],  # 8, -1
    'movable_object.barrier': ['Temporary road barrier to redirect traffic.', 'concrete barrier', 'metal barrier',
                               'water barrier'],  # 9, 9
    'movable_object.debris': ['Movable object that is left on the driveable surface.', 'tree branch', 'full trash bag'],  # 10, -1
    'movable_object.pushable_pullable': ['Object that a pedestrian may push or pull.', 'dolley', 'wheel barrow',
                                         'garbage-bin', 'shopping cart'],  # 11, -1
    'movable_object.trafficcone': ['traffic cone.'],  # 12, 8
    'static_object.bicycle_rack': ['Area or device intended to park or secure the bicycles in a row.'],  # 13, -1
    'vehicle.bicycle': ['Bicycle'],  # 14, 5
    'vehicle.bus.bendy': ['Bendy bus'],  # 15, 3
    'vehicle.bus.rigid': ['Rigid bus'],  # 16, 3
    'vehicle.car': ['Vehicle designed primarily for personal use.', 'car', 'vehicle', 'sedan', 'hatch-back', 'wagon',
                    'van', 'mini-van', 'SUV', 'jeep'],  # 17, 0
    'vehicle.construction': ['Vehicle designed for construction.', 'crane'],  # 18, 4
    'vehicle.emergency.ambulance': ['ambulance', 'ambulance vehicle'],  # 19, -1
    'vehicle.emergency.police': ['police vehicle', 'police car', 'police bicycle', 'police motorcycle'],  # 20, -1
    'vehicle.motorcycle': ['motorcycle', 'vespa', 'scooter'],  # 21, 6
    'vehicle.trailer': ['trailer', 'truck trailer', 'car trailer', 'bike trailer'],  # 22, 2
    'vehicle.truck': ['Vehicle primarily designed to haul cargo.', 'pick-up', 'lorry', 'truck', 'semi-tractor'],  # 23, 1
    'flat.driveable_surface': ['Paved surface that a car can drive.', 'Unpaved surface that a car can drive.'],  # 24, 10
    'flat.other': ['traffic island', 'delimiter', 'rail track', 'small stairs', 'lake', 'river'],  # 25, 11
    'flat.sidewalk': ['sidewalk', 'pedestrian walkway', 'bike path'],  # 26, 12
    'flat.terrain': ['grass', 'rolling hill', 'soil', 'sand', 'gravel'],  # 27, 13
    'static.manmade': ['man-made structure', 'building', 'wall', 'guard rail', 'fence', 'pole', 'drainage', 'hydrant',
                       ...
                       'stairs'],  # 28, 14
    'static.other': [
        'Points in the background that are not distinguishable, or objects that do not match any of the above labels.'],  # 29, -1
    'static.vegetation': ['bushes', 'bush', 'plants', 'plant', 'potted plant', 'tree', 'trees'],  # 30, 15
    'vehicle.ego': [
        'The vehicle on which the cameras, radar and lidar are mounted, that is sometimes visible at the bottom of the image.'],  # 31, -1
}
cls2idx = [
