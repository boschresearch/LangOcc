...
reducer_cfg = [1, 0, 'None', 'U', 128, True]
...
model = dict(
    ...
    renderer=dict(
        ...
        render_modules=[
            dict(
                type='LanguageRenderModule',
                loss_cfg=dict(type='CosSimWeightedMSELoss', loss_weight=1., reduce_by= 'sum', stop_cossim=True)
            )
        ],
            pc_range=pc_range,
            samples_per_ray=100,
    ...
    language_decoder=dict(
        ...
        num_hidden_layers=3,
        embed_dims=hidden_dim,
        num_classes=128, # output dimension of language features
        final_act_cfg=None
    ),
)
...
train_pipeline = [
    ...
    dict(type='GenerateRaysMaskCLIP', embeddings_root=embeddings_root, model=embedding_model, rays_root=rays_root, 
         num_rays=total_rays // batch_size, render_frame_ids=render_frame_ids, reducer_cfg=reducer_cfg),
